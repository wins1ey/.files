...
{
	FILE *pp = popen("echo -e \"no\nrestart\nyes\nlock\nreboot\nshutdown\" | dmenu -i -sb red -p \"Quit DWM?\"", "r");
	if(pp != NULL) {
		char buf[1024];
...
setup(void)
{
	...
	XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
		PropModeReplace, (unsigned char *) "LG3D", 4); /* Changed to solve broken java GUIs */
...
main(int argc, char *argv[])
{
	...
	XCloseDisplay(dpy);
    if (restart == 1) {
        system("systemctl restart --now --user gpu-screen-recorder");
        execlp("dwm", "dwm", NULL);
    }
    else if (shutdown == 1) {
